...
class CrossModel(nn.Module):
    ...
    def decode_greedy(self, encoder_states, xs_rev, encoder_states_kg, encoder_states_db, attention_kg, attention_db, attention_rv, bsz, maxlen):
        """
        Greedy search
        ...
        for i in range(maxlen):
            ...
            #############################
            # use xs_rev to get a mask4_rev on the dictionary
            flag = 0
            ...
            #print(copy_latent_rev.shape)
            con_logits_rev = self.representation_bias_rev(copy_latent_rev)*mask4_rev.unsqueeze(1)
            #print(con_logits_rev.shape)
            #############################

            sum_logits = voc_logits + con_logits + con_logits_rev #* (1 - gate)
    ...
    def decode_forced(self, encoder_states, xs_rev, encoder_states_kg, encoder_states_db, attention_kg, attention_db, attention_rv, ys):
        """
        ...
        #logits = self.output(latent)
        # print(self.representation_bias(copy_latent).shape, self.mask4.unsqueeze(0).unsqueeze(0).shape)
        ...
        sum_logits = logits+con_logits+con_logits_rev#*(1-gate)
    ...
    def forward(self, xs, ys, mask_ys, concept_mask, db_mask, reviews_mask, seed_sets, labels, con_label, db_label, entity_vector, rec, test=True, cand_params=None, prev_enc=None, maxlen=None,
                bsz=None):
        """
        ...
        # xs.shape: 32 x 256
        # encoder_states: embedding: 32 x 256 x 300
        #                 mask: 32 x 256
        ...
        if test == False:
            ...
            scores, preds = self.decode_forced(
                encoder_states, xs_rev,
                kg_encoding, db_encoding, con_user_emb, db_user_emb, rev_atten_emb, 
                mask_ys
            )
