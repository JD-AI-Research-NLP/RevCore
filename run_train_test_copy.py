...
class TrainLoop_fusion_rec():
    ...
    def train(self):
        ...
        for i in range(3):
            ...
            train_dataset_loader = torch.utils.data.DataLoader(dataset=train_set,
                                                            ...
                                                            shuffle=False)
            num=0
            for context,c_lengths,response,r_length,mask_response,mask_r_length,entity,entity_vector,movie,concept_mask,dbpedia_mask,reviews_mask,concept_vec, db_vec,rec in tqdm(train_dataset_loader):
                ...
                scores, preds, rec_scores, rec_loss, gen_loss, mask_loss, info_db_loss, _=self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask,seed_sets, movie, concept_vec, db_vec, entity_vector.cuda(), rec, test=False)
        ...
        for i in range(self.epoch):
            ...
            recall_top1 = output_metrics_rec['recall@1']
            rec_loss = output_metrics_rec['loss']
            writer.add_scalar('R@1', recall_top1, i)
            writer.add_scalar('loss', rec_loss, i)
    ...
    def val(self, is_test=False):
        ...
        val_dataset_loader = torch.utils.data.DataLoader(dataset=val_set,
                                                           ...
                                                           shuffle=False)
        recs=[]
        for context, c_lengths, response, r_length, mask_response, mask_r_length, entity, entity_vector, movie, concept_mask, dbpedia_mask,reviews_mask, concept_vec, db_vec, rec in tqdm(val_dataset_loader):
            with torch.no_grad():
                seed_sets = []
                batch_size = context.shape[0]
                for b in range(batch_size):
                    seed_set = entity[b].nonzero().view(-1).tolist()
                    seed_sets.append(seed_set)
                scores, preds, rec_scores, rec_loss, _, mask_loss, info_db_loss, info_con_loss = self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask,reviews_mask, seed_sets, movie, concept_vec, db_vec, entity_vector.cuda(), rec, test=True, maxlen=20, bsz=batch_size)
...
class TrainLoop_fusion_gen():
    ...
    def train(self):
        ...
        for i in range(self.epoch*3):
            ...
            for context, c_lengths, response, r_length, mask_response, mask_r_length, entity,entity_vector, movie, concept_mask, dbpedia_mask, reviews_mask, concept_vec, db_vec, rec in tqdm(train_dataset_loader):
                ...
                scores, preds, rec_scores, rec_loss, gen_loss, mask_loss, info_db_loss, info_con_loss=self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask, seed_sets, movie, concept_vec, db_vec, entity_vector.cuda(), rec, test=False)
    ...
    def val(self,is_test=False):
        ...
        for context, c_lengths, response, r_length, mask_response, mask_r_length, entity, entity_vector, movie, concept_mask, dbpedia_mask, reviews_mask, concept_vec, db_vec, rec in tqdm(val_dataset_loader):
            ...
            with torch.no_grad():
                ...
                for b in range(batch_size):
                    seed_set = entity[b].nonzero().view(-1).tolist()
                    seed_sets.append(seed_set)
                _, _, _, _, gen_loss, mask_loss, info_db_loss, info_con_loss = self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask, seed_sets, movie, concept_vec, db_vec, entity_vector.cuda(), rec, test=False)
                scores, preds, rec_scores, rec_loss, _, mask_loss, info_db_loss, info_con_loss = self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask, seed_sets, movie, concept_vec, db_vec, entity_vector.cuda(), rec, test=True, maxlen=50, bsz=batch_size)
        ...
        f=open('output_test_50_copy.txt','w',encoding='utf-8')
...
if __name__ == '__main__':
    ...
    print(vars(args))
    writer = SummaryWriter(args.tf_log)
