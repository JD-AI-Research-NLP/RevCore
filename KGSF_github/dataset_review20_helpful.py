...
class dataset(object):
    def __init__(self,filename,opt):
        ...
        with open('movie2tokenreview_helpful.pkl','rb') as f:
            dict_movieid2reviewtoken = pkl.load(f)
        self.reviews = dict_movieid2reviewtoken
    ...
    def prepare_word2vec(self):
        ...
        for word in word2index:
            ...
            if word in self.movie_wordset:
                mask4movie[idx] = 1
        print(mask4key.shape, mask4key.sum())
        print(mask4movie.shape, mask4movie.sum())
...
#             print('1111111')
        ...
        for db in dbpedia_mask:
            if db!=0:
                db_vec[db]=1
        #print('data with review')
        return context, c_lengths, response, r_length, mask_response, mask_r_length, entity_vec, entity_vector, movie, np.array(concept_mask), np.array(dbpedia_mask), concept_vec, reviews_mask, db_vec, rec
